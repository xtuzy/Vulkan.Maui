...
{
    public class HelloTriangleGame : GameBase
    {
        #region Game method
        public override void OnGraphicsDeviceCreated()
        {
            base.OnGraphicsDeviceCreated();
            InitVulkan();
            DrawFrame(16);
        }

        public override void DrawFrame(float deltaMillisecond)
        {
            DrawFrame(deltaMillisecond);
        }

        public override void OnViewResize()
        {
            base.OnViewResize();
            var frameSize = GpuView.FramebufferSize;
            OnFramebufferResize(new Vector2D<int>((int)frameSize.X, (int)frameSize.Y));
        }
        ...
        private IGpuView _window => GpuView;
        ...
        private Vk _vk => GpuView.AppInfo.Vk;
        ...
        private void InitWindow()
        {
            ...
            if (_window.AppInfo?.VkSurface is null)
            {
        ...
        private unsafe void CreateInstance()
        {
            ...
            var extensions = _window.AppInfo.VkSurface!.GetRequiredExtensions(out var extCount);
        ...
        {
            _surface = _window.AppInfo.VkSurface!.Create<AllocationCallbacks>(_instance.ToHandle(), null).ToSurface();
        }
        ...
        private unsafe QueueFamilyIndices FindQueueFamilies(PhysicalDevice device)
        {
            ...
            _vk.GetPhysicalDeviceQueueFamilyProperties(device, ref queryFamilyCount, null);
            QueueFamilyProperties[] queueFamilies = new QueueFamilyProperties[queryFamilyCount];
            _vk.GetPhysicalDeviceQueueFamilyProperties(device, ref queryFamilyCount, out queueFamilies[0]);
        ...
        {
            Vector2D<int> framebufferSize = new((int)_window.FramebufferSize.X, (int)_window.FramebufferSize.Y);

            while (framebufferSize.X == 0 || framebufferSize.Y == 0)
            {
                framebufferSize = new((int)_window.FramebufferSize.X, (int)_window.FramebufferSize.Y);
                //_window.DoEvents();
            }
...
";
        ...
        private unsafe void CreateGraphicsPipeline()
        {
            ...
            try
            {
                vertShaderCode = LoadEmbeddedResourceBytes("VulkanTriangle.shader.vert.spv");
                fragShaderCode = LoadEmbeddedResourceBytes("VulkanTriangle.shader.frag.spv");
            }
            catch (Exception ex) 
            {
                vertShaderCode = Encoding.UTF8.GetBytes(vert);
                fragShaderCode = Encoding.UTF8.GetBytes(frag);
            }
        ...
        internal byte[] LoadEmbeddedResourceBytes(string path)
        {
            using (var s = typeof(HelloTriangleGame).Assembly.GetManifestResourceStream(path))
            {
